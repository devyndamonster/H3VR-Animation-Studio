...
namespace H3VRAnimator
{
    public class RangedSlidingPoint : MovablePoint
    {
        ...
        public Color slideColor = new Color(.5f, .5f, .5f, 0.5f);
        ...
        public override void DrawGizmos()
        {
            ...
            Popcron.Gizmos.Line(transform.position + transform.up * -maxDown, transform.position + transform.up * maxUp, slideColor);
        }
        ...
        public override void CheckForMove()
        {
            if (activeHand != null)
            {
                if (!lockPosition)
                {
                    buttonPoint.transform.position = Vector3.Project(activeHand.PointingTransform.position + activeHand.PointingTransform.forward * savedDist, transform.up);
